                    ...
                    if is_owned(fn_ret) || needs_nulls || post.is_some() {
                        ...
                        if fn_ret.is_tuple() {
                            ...
                            for (i, e) in elems.iter().enumerate() {
                                ...
                                indent(out, level);
                                let cast = if td.elems[i] == Ty::F64 { "(double*)" } else { "(long long*)" };
                                let valcast = if td.elems[i] == Ty::F64 { "(double)" } else { "(long long)" };
                                let _ = writeln!(out, "  (({})t_ret)[{}] = {}({});", cast, i, valcast, s);
                            }
        ...
        Stmt::MultiDecl { names, value, mutable, .. } => {
            ...
            for (i, n) in names.iter().enumerate() {
                if is_owned(td.elems[i]) {
                    indent(out, level);
                    let _ = writeln!(out, "{} {} = NULL;", c_ty(td.elems[i]), cname(n));
                }
            }
            let mut e = String::new();
            emit_expr(value, scope, sigs, &mut e);
            ...
            for (i, n) in names.iter().enumerate() {
                ...
                let cast = if td.elems[i] == Ty::F64 { "(double*)" } else { "(long long*)" };
                let valcast = if td.elems[i] == Ty::F64 { "(double)" } else { "(long long)" };
                let _ = writeln!(out, "  {} = {}((({})__t_t)[{}]);", cname(n), valcast, cast, i);
            }
            indent(out, level);
