...
| 版本 | 主题 | 要点 |
|---|---|---|
| v5.0 | **泛型**（单态化） ✅ **已完成（2026-09-08）** | `Map[K,V]` / `Result[T,E]` / `Option[T]` 内置泛型类型，类型级单态化落到既有 map/enum 表（hash-consing 去重、零新增 runtime）；`map[K]V` 降级为语法糖；Result/Option 构造由期望类型物化为具体枚举，解构复用 v4.9 match。用户自定义泛型/泛型函数留 v5.x（规范第 28 节） |
