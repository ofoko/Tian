...
/// 完整编译管线：源码 → Token → AST → 类型检查 → C 源码
fn compile(path: &str) -> Result<String, String> {
    let src = read_source(path);
    let toks = lexer::lex_spanned(&src).map_err(|e| e.to_string())?;
    let prog = parser::parse(toks).map_err(|e| e.to_string())?;
...
fn cmd_diagnose_json(path: &str) {
    ...
    let toks = match lexer::lex_spanned(&src) {
        Ok(t) => t,
        Err(e) => {
            println!(
                "{}",
                json("lex", e.line, &e.msg, "检查非法字符、未闭合的字符串或数字格式", &[])
            );
            exit(1);
        }
    };
    let prog = match parser::parse(toks) {
        ...
        Err(e) => {
            println!(
                "{}",
                json("parse", e.line, &e.msg, "检查语句是否以换行结束、块是否用 } 闭合", &[])
            );
            exit(1);
        }
    };
...
fn cmd_test(path: &str) {
    let src = read_source(path);
    let mut prog = match lexer::lex_spanned(&src)
...
fn cmd_build(path: &str) {
    let src = read_source(path);
    let prog = match lexer::lex_spanned(&src)
        .map_err(|e| e.to_string())
        .and_then(|t| parser::parse(t).map_err(|e| e.to_string()))
    {
        Ok(p) => p,
    ...
    match gen_native::generate_object(&prog) {
        ...
        Err(e) => {
            ...
            let cc = find_cc();
            let st = std::process::Command::new(cc).arg("-O2").arg(&c_path).arg("-o").arg(&bin_path)
                .status().unwrap_or_else(|e| { eprintln!("调用 {} 失败：{}", cc, e); exit(1); });
            if !st.success() {
                eprintln!("C 编译失败（生成的 C 代码：{}）", c_path.display());
                exit(1);
...
fn usage() {
    eprintln!("tc —— 天语言编译器 v1.1（Cranelift 原生后端）");
    eprintln!();
    eprintln!("用法：");
