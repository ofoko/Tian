...
//! 整数除零由 Cranelift 自动插入陷阱（进程崩溃退出，符合"简单诚实"原则）。
...
use crate::ast::*;
use crate::type_check::{collect_str_decls, consumes_var, is_owned, set_structs, structs, FuncSig};
...
fn compile_fn(
    ...
    for p in &f.params {
        if p.ty.map(is_owned).unwrap_or(false) {
...
fn emit_post_check(
    ...
    let pi = builder.ins().uextend(types::I32, pv);
    let what = str_ptr(ctx, builder, &format!("{} @post（第{}行）", ctx.fname, line))?;
    let ck = ctx.rt.check;
            ...
            for c in &f.contracts {
                if let Contract::Example { call, expected, line } = c {
                    ...
                    let what = str_ptr(
                        &mut ctx,
                        &mut builder,
                        &format!("{} @example（第{}行）", f.name, line),
                    )?;
                    let ck = ctx.rt.check;
...
fn emit_block(
    ...
    for (i, s) in stmts.iter().enumerate() {
        ...
        if matches!(s, Stmt::Return(..)) && i + 1 < stmts.len() {
...
fn emit_stmt(
    ...
    match s {
        ...
        Stmt::Assign { target, value, .. } => {
            match target {
                ...
                Expr::Field(base, fname) => {
                    ...
                    let (sd, idx, ft) = struct_field(bt, fname)
                    ...
                    if ft == Ty::Str {
                        ...
                        let v = coerce(builder, val, vt, Ty::Str)?;
        ...
        Stmt::Return(e, _) => {
            let val = match e {
                Some(expr) => {
                    let val = if fn_ret == Ty::Str {
                        // 天权：返回即移交所有权（规范 11.2.1）；
                        // 先求值（字面量/tos 结果复制），再接管
                        emit_bind_native(expr, ctx, builder)?
                    } else {
                        let (v, vt) = emit_expr(expr, ctx, builder)?;
                        coerce(builder, v, vt, fn_ret)?
                    };
...
fn load_field(builder: &mut FunctionBuilder, base: Value, offset: i32, ft: Ty) -> Result<Value, String> {
    match ft {
        ...
        Ty::Struct(i) => Ok(builder.ins().load(ptr_ty(), MemFlags::new(), base, offset)),
